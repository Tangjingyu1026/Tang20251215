...
extern "C" {
	...
	struct IO_InitStruct_Tx				     //1.初始化引脚的结构体（发送）
	{
		uint8_t Pin;
		uint8_t Mode;
		uint8_t Pull;
	...
	struct IO_InitStruct_Rx  				 //2.初始化引脚的结构体（接收）
	{
		uint8_t Ret;
	};
	typedef struct IO_InitStruct_Rx IO_InitStruct_Rx_t;

	struct IO_ReadStruct_Tx     			 //3.读取引脚状态的结构体（发送）
	{
		uint8_t Pin;
	};
	typedef struct IO_ReadStruct_Tx IO_ReadStruct_Tx_t;

	struct IO_ReadStruct_Rx                  //4.读取引脚状态的结构体（接收）
	{
		uint8_t Ret;						//0为正常；<0为异常
		uint8_t PinState; 					//引脚状态。0低电平。1高电平
	};
	typedef struct IO_ReadStruct_Rx IO_ReadStruct_Rx_t;

	struct IO_WriteStruct_Tx 				//5.写入状态的结构体（发送）
	{
		uint8_t Pin; 
		uint8_t PinState;
	};
	...
	struct IO_WriteStruct_Rx				//6.写入状态的结构体（接收）	 
	{
		uint8_t Ret;						//0为正常；<0为异常
	};
	...
	int WINAPI IO_InitMultiPin(int SerialNumber, IO_InitStruct_Tx_t* TxStruct, IO_InitStruct_Rx_t* RxStruct, int Number);//批量初始化多个引脚工作模式
	int WINAPI IO_ReadMultiPin(int SerialNumber, IO_ReadStruct_Tx_t* TxStruct, IO_ReadStruct_Rx_t* RxStruct, int Number);//批量读取多个引脚状态
	int WINAPI IO_WriteMultiPin(int SerialNumber, IO_WriteStruct_Tx_t* TxStruct, IO_WriteStruct_Rx_t* RxStruct, int Number);//批量写入输出状态
